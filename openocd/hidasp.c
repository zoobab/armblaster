...
//	for JTAG
#define	JTAG_DUMP		0		// JTAG COMMAND をダンプ.
#define	POKE_DUMP		0		// PEEK/POKE COMMAND をダンプ.
#define	DEBUG_STREAM	0		// 1: BitBang データ列をデバッグする.
...
/*********************************************************************
 *	USBターゲットに対して送信した、PIC用のBitBang命令をダンプする.
 *********************************************************************
 */
...
/*********************************************************************
 *	USBターゲットに対して、PIC用のBitBang命令を送信する.
 *********************************************************************
 */
...
#if	1
//
//	以下の処理は、基本ポートI/Oにつき低速.
//	  また、jcmdバッファに未処理のデータが存在していてはいけない.
//		(PORT値設定の順序が逆になる)
//

static	int	bb_data=dirPGD;	// TDOのみ入力にする.
/*********************************************************************
 *	TCK ,TDI ,TDO ,TMS ポートの入力データを読み取る.
 *********************************************************************
...
int get_portb(void)
{
	...
	c = result[1];
#if	POKE_DUMP			// PEEK/POKE COMMAND をダンプ.
	if(c & bitPGD) 	printf("+");
	else 			printf("_");
#endif
...
/*********************************************************************
 *	TCK ,TDI ,TDO ,TMS ポートの出力データをセットする.
 *********************************************************************
  ATtiny2313        ARM基板 JTAG端子
         PB7  ------------- TCK
         PB6  ------------- TDI
