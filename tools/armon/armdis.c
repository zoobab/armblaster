...
void floatformat_to_double(const void *fmt,
...
//	{ARM_EXT_V4T, 0x4800, 0xF800, "ldr%c\t%8-10r, [pc, #%0-7W]\t(%0-7a)"},  /* TODO: Disassemble PC relative "LDR rD,=<symbolic>" */
	{ARM_EXT_V4T, 0x4800, 0xF800, "ldr%c\t%8-10r, [pc, #%0-7W]\t; %8-10r = #%0-7j"},  /* TODO: Disassemble PC relative "LDR rD,=<symbolic>" */
	/* format 9 */
...
//	{ARM_EXT_V4T, 0xA000, 0xF800, "add%c\t%8-10r, pc, #%0-7W\t(adr %8-10r, %0-7a)"},
	{ARM_EXT_V4T, 0xA000, 0xF800, "add%c\t%8-10r, pc, #%0-7W\t(adr %8-10r, #%0-7j)"},
...
//	=================================================
//	PC相対フェッチした即値のアドレスを記憶する.
//	=================================================
//
#define	MAX_RELTAB	256		// 2のべき
#define	PC_RELMASK	(MAX_RELTAB-1)
int	pc_reltab[MAX_RELTAB];
int	pc_relidx=0;

//	=================================================
//	PC相対フェッチした即値のアドレスをご破算にする.
//	=================================================
//
void reltab_init(void)
{
	int i;
	for(i=0;i<MAX_RELTAB;i++) pc_reltab[i]=0;
}

//	=================================================
//	PC相対フェッチする即値をターゲット側から取得する.
//	=================================================
//
int	peek_imm_pcrel(int adr,int width)
{
	pc_reltab[pc_relidx]=adr;

	// ラウンドロビン.
	pc_relidx++;
	if( pc_relidx >= MAX_RELTAB) {
		pc_relidx = 0;
	}

	return UsbPeek(adr,width);
}
//	=================================================
//	PC相対フェッチした即値のアドレスを検索する.
//	=================================================
//
int	find_imm_pcrel(int adr)
{
	int i;

	if(adr==0) return 0;	// 例外: ０番地はデータではない. 

	// 今のところリニアサーチ.
	for(i=0;i<MAX_RELTAB;i++) {
		if( adr == pc_reltab[i] ) {	return 2; }
	}
	return 0;
}
int	is_a_vector(int adr)
{
#ifdef	LPC2388
	//
#else
#ifdef	LPC1343
	if((adr >= 0) 		   && (adr < 0x11c)) 	  {return 2;}
	if((adr >= 0x1fff0000) && (adr < 0x1fff0104)) {return 2;}
#else
	if((adr >= 0) 		   && (adr < 0x11c)) 	  {return 2;}
	if((adr >= 0x08000000) && (adr < 0x0800011c)) {return 2;}
	if((adr >= 0x08002000) && (adr < 0x0800211c)) {return 2;}
#endif
#endif
	return 0;
}
...
#else
// =========== ARM32: PC相対アドレスの実効番地を即値とみなして表示.
		func (stream, "\t; = #");
		//info->print_address_func (offset, info);
		{
			int adr = offset;
			int imm = peek_imm_pcrel(adr,MEM_WORD);
...
// =========== PC相対アドレスの実効番地を即値とみなして表示.
						case 'j':
							/* PC-relative address -- the bottom two
							   bits of the address are dropped
							   before the calculation.  */
							{
								int adr = (((pc + 4) & ~3) + (reg << 2));
								int imm = peek_imm_pcrel(adr,MEM_WORD);
								info->print_address_func(imm,info);
							}
							break;
